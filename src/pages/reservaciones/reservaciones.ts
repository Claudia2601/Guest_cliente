...
export class ReservacionesPage {
  ...
  //   console.log("Esta es la zona",this.data.zona);
  ...
  alertConsumo(uid) {
    const zona = this.data.zona;
    console.log("Esta es la zona",uid);
  ...
  reservacionAdd() {
    ...
    this._providerReserva.saveReservacion(info).then((respuesta: any) => {
      ...
      if (this.compartir != undefined) {
        ...
        this._providerReserva.saveCompartirPropio(this.telefono, respuesta.idReservacion, this.uid).then((respuesta: any) => {
        ...
        this.compartir.forEach(data => {
          ...
          this._providerReserva.buscarPlayerid(this.telefono3).subscribe(players => {
            this.players = players[0].playerID;
            this.phoneuser = players[0].phoneNumber;
            console.log('playersidsolo', this.players);
            console.log('numero solo', this.phoneuser);
            //consulta para insertar cada numero que viene del seleccionar varios en compartidas
            this._providerReserva.saveCompartir(this.phoneuser, respuesta.idReservacion, this.uid, this.players).then((respuesta2: any) => {
