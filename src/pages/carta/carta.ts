...
export class CartaPage {
  ...
  constructor(
    ...
    this.afs.collection('cartas').valueChanges().subscribe(data => {
      this.totalcartas = data;
      console.log("estas son las cartas", this.totalcartas);

      var cartassarreglo = [];
      // var esferasarregloAc = [];
      // var esferasarregloEstado = [];

      this.totalcartas.forEach(element => {
        const id = element.uid
        const category = element.categoria

        console.log('Datos de las cartas', id, category)

        this.afs.collection('cartas', ref => ref.where('categoria', '==', category)).valueChanges().subscribe(data => {
          this.globalcategory = data;
          console.log("estos son mis categorias seleccionadas", this.globalcategory);
          console.log("estos son mis categorias seleccionadas", this.globalcategory.length);

          if (this.globalcategory.length >= 1) {
            cartassarreglo.push(id);
            console.log("categorias", cartassarreglo.join(", "));
            console.log("categorias total", cartassarreglo);

            this.categorias_seleccionadas = this.globalcategory;
            console.log("this.categorias_seleccionadas", this.categorias_seleccionadas.categoria);
